...
config :api, ApiWeb.Endpoint,
  ...
  live_view: [signing_salt: "WBq+5Uxz"]
...
config :data, DataWeb.Endpoint,
  ...
  adapter: Bandit.PhoenixAdapter,
  render_errors: [
    formats: [html: DataWeb.ErrorHTML, json: DataWeb.ErrorJSON],
    layout: false
  ],
  pubsub_server: Data.PubSub,
  live_view: [signing_salt: "4YOyBeaB"]
...
config :esbuild,
  ...
  data: [
    args:
      ~w(js/app.js --bundle --target=es2017 --outdir=../priv/static/assets --external:/fonts/* --external:/images/*),
    cd: Path.expand("../apps/data/assets", __DIR__),
    env: %{"NODE_PATH" => Path.expand("../deps", __DIR__)}
  ]
